...
However, writing these by hand is also not a good idea because they are intended
to be auto-generated by `xgettext` and will be overwritten by `msgmerge`.
So how do you get `xgettext` to auto-generate these comments? `xgettext` has an
[`-c/--add-comments`](https://www.gnu.org/software/gettext/manual/gettext.html#index-_002dc_002c-xgettext-option)
flag which tells the program to scan for comments preceding the line in which a
translatable string is found.
